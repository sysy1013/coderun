...
module.exports = async (req, res) => {
    ...
    const { questionId } = req.query; // Changed from req.params to req.query

    // 필요한 값이 없을 때 보내주는 response
    if (!accesstoken || !questionId) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
    ...
    try {
        ...
        const repeatQuestion = await questionDB.repeatQuestionSolve(client, userId, questionId);

        // 성공적으로 users를 가져왔다면, response를 보내줍니다.
        res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_ALL_USERS_SUCCESS, repeatQuestion));
